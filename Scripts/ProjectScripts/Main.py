...
print(f'cfo140_last_row: {cfo140_last_row}')
print(f'rnpd_last_row: {rnpd_last_row}')
...
working_sheet_cfo140_file.cell(cfo140_headers_row_index, result_colunm_index).fill = PatternFill(DEFAULT_FILL_TYPE, start_color=DEFAULT_COLOR)
...
cfo140_column_index_nomer_dogovora = MyFunctions.get_index_any_from_list(cfo140_column_names, ('Системный номер договора', 'Системный № Договора', ' Системный № Договора'))
...
cfo140_column_index_Kontragent = MyFunctions.get_index_any_from_list(cfo140_column_names, ('Наименование контрагента', 'Поставщик'))
cfo140_column_index_Summa = MyFunctions.get_index_any_from_list(cfo140_column_names, ('Сумма без НДС (руб)', 'Плановая сумма поставки (без НДС)', 'Плановая сумма поставки (без НДС)'))
cfo140_column_index_Data = MyFunctions.get_index_any_from_list(cfo140_column_names, ('Дата фактического оказания услуг', 'Дата факт. оказ. Услуги'))
...
for cfo140_row_index, cfo140_row in enumerate(working_sheet_cfo140_file, start = 1):
    if(cfo140_row[cfo140_column_index_BP].value not in rnpd_OE_values):
        continue
...
for cfo140_row_index, cfo140_row in enumerate(working_sheet_cfo140_file, start = 1):
    ...
    if (cfo140_row_index > cfo140_headers_row_index and cfo140_row_index <= cfo140_last_row):  # пропускаю первую пустую строку и заголовок в файле
        if (cfo140_row[cfo140_column_index_BE].value not in rnpd_OE_values):
            ...
            cfo140_row[cfo140_column_index_Kommentariy].fill = PatternFill(DEFAULT_FILL_TYPE, start_color='00FF0000')
            continue
        ...
        # searching_set = set(matched_item_list)
        # if(cfo140_row_key in searching_set):

        if (cfo140_row_key in matched_item_list):
            row_key_matches_count = list(map(lambda x: x.key, matched_item_list)).count(cfo140_row_key)
            print(f'Row key: {cfo140_row_key}, matches count: {row_key_matches_count}')
            if (row_key_matches_count > 1):
                ...
                cfo140_row[
                    cfo140_column_index_Kommentariy].value = f"Найдено более одного сопадения в файле РНПД. Строки: {', '.join(map(str, rows_numbers))}!"
                cfo140_row[cfo140_column_index_Kommentariy].fill = PatternFill(DEFAULT_FILL_TYPE,
            ...
            cfo140_row[cfo140_column_index_Kommentariy].value = f"В файле РНПД не найдено совпадений. Добавлена строка: {rnpd_file_result_ws.max_row}!"
        ...
        if (row_key_matches_count > 1):
            ...
            if(matched_item.old_summ != matched_item.new_summ):
                rnpd_cell = rnpd_file_result_ws.cell(matched_item.rpnd_row_index +1,  rnpd_column_index_summa_raspredelenija +1)
                rnpd_cell.value = matched_item.new_summ
                rnpd_cell.fill = PatternFill(DEFAULT_FILL_TYPE, start_color=DEFAULT_COLOR)

                cfo140_row[cfo140_column_index_Kommentariy].value = f"В файле РНПД отредактирована сумма по строке: {matched_item.rpnd_row_index + 1}. Старая сумма: {matched_item.old_summ}. Новая сумма {matched_item.new_summ}."
            else:
                cfo140_row[cfo140_column_index_Kommentariy].value = f"В файле РНПД сумма по строке: {matched_item.rpnd_row_index + 1} совпадает."
            ...
            SHPP = cfo140_row[cfo140_column_index_SHPP].value
            SZ = cfo140_row[cfo140_column_index_statja_zatrat].value
            PROGRAMMA = cfo140_row[cfo140_column_index_Programma].value
            SUBJECT = str(cfo140_row[cfo140_column_index_Subject].value).rjust(6, "0")
            BP = str(cfo140_row[cfo140_column_index_BP ].value).rjust(6, "0")
            MVZ = cfo140_row[cfo140_column_index_MVZ].value
